...
const updatePost = () => {
    store.dispatch("posts/updatePost", updateState.value)
    closeEditPostModal()
}
</script>
<template>
    <div class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div @click="closeEditPostModal" class="fixed  inset-0 bg-black bg-opacity-75 transition-opacity"></div>
            ...
            <div class="flex  items-end justify-center p-2  sm:items-center sm:p-0">
                <div
                    ...
                    <div class="border-b md:px-6 p-3 md:py-3 border border-opacity-70 flex justify-between items-center">
                        <h3 class=" md:text-xl " id="modal-title">Gönderiyi Düzenle</h3>
                        <button @click="closeEditPostModal"
                    ...
                    <div class="md:pt-2 pb-4 px-3 sm:py-6 ">
                        <div class="mt-3 pr-3 sm:mt-0 sm:ml-4 flex flex-col space-y-4  ">
                            ...
                            <textarea v-model="updateState.content" cols="30"
                                placeholder="Ne hakkında konuşmak istiyorsunuz?" class="placeholder:text-muted outline-none"
                                rows="10"></textarea>
                        </div>
                    </div>
                    <div
                        class="bg-white  border border-opacity-70 px-4 py-3 space-x-2 flex justify-end items-center sm:px-6">
                        <button :disabled="!props.post" @click="updatePost"
                            class="bg-primary disabled:bg-gray-300 disabled:text-gray-400 rounded-full py-1 px-3 text-white active:bg-[#09223b] hover:bg-[#004182] transition-all duration-300 font-bold">Kaydet</button>
                    </div>
